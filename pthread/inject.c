...
/* 
 * Watch out for RIP relative addressing when modifying the assembly code.
 */
char injectedCode[] =
  // "\xCC"                         // int3
  ...
  "\xEB\xFE"                        // jmp      0x0
  /* 
   * Never reached. This stub thread is going to be "reaped" by thread_terminate(). 
   */
  "\xC3"                            // ret
  ...
  /*
  "\xbf\x14\x00\x00\x00"            // mov      edi, 20
  ...
  */
  ...
  "\x48\x8D\x3D\x1D\x00\x00\x00"    // lea      rdi, qword ptr [rip + 0x1d] 
...
int inject(pid_t pid, const char *lib)
{
  ...
  */
  ...
  for (i = 0; i < 0x100; i++) {
    ...
    if (memcmp(possiblePatchLocation, "SLEEP___", 8) == 0) {
      printf("sleep @%llx\n", addrOfSleep);
  ...
  for (;;) {
    ...
    /*
     * The stub thread executes "mov rax, 0xD13" instruction, then enter the endless loop.
     * Inject process terminates it by thread_terminate(). 
     */
